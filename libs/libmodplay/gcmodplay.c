...
static u8 audioBuf[2][SNDBUFFERSIZE];

static sys_semaphore_t player_semaid;
static sys_semaphore_attribute_t player_sema_attr;
static sys_ppu_thread_t h_player;
...
static void player(void *arg)
{
	...
	while(sndPlaying==TRUE) {
        ...
        sys_semaphore_wait(player_semaid, 0);
	...
	sys_semaphore_destroy(player_semaid);
...
if(it_have_samples==0) {it_have_samples=1;sys_semaphore_post(player_semaid, 1);return;}
...
static s32 SndBufStart(MODSNDBUF *sndbuf)
{
    ...
    if (sys_ppu_thread_create(&h_player, (void *) player, (u64) 0, 900, STACKSIZE,
        THREAD_JOINABLE, "Modplayer") >= 0) {
...
static void SndBufStop()
{
    ...
    sys_semaphore_post(player_semaid, 1);

	sys_ppu_thread_join(h_player, NULL);
...
void MODPlay_Init(MODPlay *mod)
{
	...
	memset(&player_sema_attr, 0, sizeof(sys_semaphore_attribute_t));

    player_sema_attr.attr_protocol = 0x2;
    player_sema_attr.attr_pshared  = 0x2 << 8;

    sys_semaphore_create(&player_semaid, &player_sema_attr, 0, 1);
